...
var buildExtensionPath = path.join(__dirname, '_build');
...
target.build = function() {
    ...
    copyExtensionResources(__dirname, buildExtensionPath);
...
target.publish = function() {
    ...
    var pkgVer = JSON.parse(fs.readFileSync(path.join(__dirname, 'vss-extension.json'))).version;

    // publish the vsix package
    ensureTool('tfx');
    run(`tfx extension publish --vsix jamestupper.tppr-vsts-tasks-${pkgVer_}.vsix --auth-type pat --token `)
}
