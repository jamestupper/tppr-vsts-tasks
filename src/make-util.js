...
var shclean = function (target) {
    shell.find(target).filter(function (file) {
        var parts =  file.split('/');
        return parts[parts.length - 1].match(/. ./);
    }).forEach(function (file) {
        shell.mv(file, file.replace(' ', '_'));
    });

    shellAssert();
}
exports.shclean = shclean;
//------------------------------------------------------------------------------
...
var getExternals = function (externals, destRoot) {
    ...
    if (externals.hasOwnProperty('archivePackages')) {
        ...
        archivePackages.forEach(function (archive) {
            ...
            // remove spaces from folders and files
            shclean(archiveDest);
        });
    }
